...
.controller('OwnCoursesCtrl', function($scope, auth, UserCoursesService, UsersService, CoursesService){
  ...
  function refreshUserCourses() {
    ...
    .then((data) => {
      ...
      .then((data) => {
        const userCoursesTable = data;
        $scope.userCourses = [];
        for (let i = 0; i < userCoursesTable.length; i++) {
          userCourseIds[i] = userCoursesTable[i].id_curso;
        }
        // Sorts the Course IDs array
        const sortedUserCourseIds = userCourseIds.sort();
        CoursesService.list()
        .then((data) => {
          // Sorts the courses by course id
          const courses = _.sortBy(data, course => course.id_curso);
          // Sorts the courses by course id
          const sortedUserCoursesTable = _.sortBy(userCoursesTable, userCourse => userCourse.id_curso);
          // Loop that adds the course to the user
          for (let i = 0; i < sortedUserCoursesTable.length; i++) {
            if (courses[i].id_curso === sortedUserCourseIds[i]) {
              $scope.userCourses[i] = courses[i];
              // This field is for tracking the type of user (teacher, student)
              $scope.userCourses[i]['tipo_relacion'] = sortedUserCoursesTable[i].tipo_relacion;
            }
          }
        })
      })
...
.controller('CourseDetailCtrl', function($scope, $stateParams, $state, auth,
  ...
  .controller('ClassesCtrl', function($scope, ClassesService, auth, $stateParams) {
    ...
    $scope.isGroupShown = (week) => $scope.shownGroup === week;
  })
  // Tests Controller
  .controller('TestsCtrl', function($scope, TestsService, auth, $stateParams, $ionicHistory) {
    ...
    function refreshTests() {
      ...
      TestsService.list(courseId)
      .then((data) => {
        $scope.tests = data;
        $scope.tests = _.sortBy($scope.tests, test => test.semana);
        $scope.courseTests = [];
        let testNames = [];
        let id = [];
        let weekIndex = 1;
  ...
  .controller('TestDetailCtrl', function($scope, $stateParams, TestsService,
    ...
    function refreshTest() {
      TestsService.get(testId).then((data) => {
        $scope.test = data;
    ...
    function refrestQuestions() {
      // Loading spinner starts
      $scope.loading = true;
      TestsService.listQuestions(testId).then((data) => {

        const questions = _.sortBy(data, question => question.id_pregunta);
          ...
          $scope.submit = () => {
            ...
            if (Object.keys($scope.data).length == multipleQuestionsLength
              ...
              .then((userData) => {
                ...
                TestsService.getAttempts(testDataGet).then((attemptData) => {
                  ...
                  if (attempts === 0) {
                    ...
                    TestsService.createAttempt(testData).then((createAttemptData) => {
                      console.log('Attempt created');
                    });
                  } else {
                    attempts ++;
                    testData.attempts = attempts;
                    TestsService.updateAttempt(testData).then((updateAttemptData) => {
  ...
  .controller('ClassDetailCtrl', function($scope, $stateParams, ClassesService, $sce) {
    ...
    function refreshClass() {
      // Loading spinner starts
      $scope.loading = true;
      ClassesService.get($stateParams.classId).then((data) => {
        $scope.class = data;
    ...
    $scope.getTrustedHTML = (str) => $sce.trustAsHtml(str);
